...
get :: UpDown -> Int
get (UpDownIncreasing a) = a
get (UpDownDecreasing a) = a 

-- tick increases an increasing counter by one or decreases a
-- decreasing counter by one
tick :: UpDown -> UpDown
tick (UpDownIncreasing a) = UpDownIncreasing (a+1)
tick (UpDownDecreasing a) = UpDownDecreasing (a-1) 

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (UpDownIncreasing a) = UpDownDecreasing a
toggle (UpDownDecreasing a) = UpDownIncreasing a  
...
mapTree _ Leaf = Leaf 
mapTree f (Node n a b) = Node (f n) (mapTree f a) (mapTree f b)
...
insertL a (Node b x y) = Node b (insertL a x ) y 
...
measure Leaf = Leaf
measure (Node _ a b) = Node(val x + val y + 1) x y
  where val Leaf = 0
        val (Node v _ _) = v
        x = measure a 
        y = measure b     
...
foldTree f x Leaf = x
foldTree f x (Node v a b) = f v (foldTree f x a) (foldTree f x b)
...
rgb Blue = [0,0,1]
rgb Green = [0,1,0]
rgb (Mix a b) = map change $ zipWith (+) (rgb a) (rgb b)
  where change a = min a 1
rgb (Darken x y) = map (*change) (rgb y)
  where change = 1-x 
