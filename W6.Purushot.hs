...
split s = case break (==' ') s of (x,' ':xs) -> Just(x,xs)
                                  _          -> Nothing 
...
myDrop mi ml = do mi' <- mi
                  ml' <- ml
                  if ((mi' > length ml') || (mi' < 0)) then Nothing else return $ drop mi' ml' 
...
lengthAndSum (x:xs) = do old <- lengthAndSum xs  
                         (modify (+x))
                         return $ old+1
...
oddUpdate x = modify (oddUpdate' x)  
  where oddUpdate' x [] = [x] 
        oddUpdate' x (y:ys)
               | (x == y) = filter (/=x) ys 
               | otherwise = (oddUpdate' x ys) ++ [y] 
...
ifM opBool opThen opElse = do x <- opBool 
...
mapM2 op _ [] = return []
mapM2 op [] _ = return []
mapM2 op (x:xs) (y:ys) = do z <- op x y
                            tail' <- mapM2 op xs ys
                            return $ (z:tail')   
...
dfs cities i = do s <- get
                  when (not (elem i s)) $ 
                     do modify(i:)
                        let connected_cities = cities !! i
                        mapM_ (dfs cities) connected_cities 
...
orderedPairs xs = do i <- [0..length xs -1] 
                     j <- [0..length xs -1]
                     let a = xs !! i 
                         b = xs !! j
...
fsum acc x = do unwrap_list <- get
                if elem x unwrap_list then return acc else modify (x:) >> return (x+acc)
------------------------------------------------------------------------------
