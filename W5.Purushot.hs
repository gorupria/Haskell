...
allEqual [] = True
allEqual (x:xs) = if (xs /= []) then (if (x == head xs) then allEqual xs else False) else True   
...
incrementKey k xs = map (\x -> if (k == fst x) then (k, (snd x) + 1) else x) xs
...
instance Eq ITree where
  ILeaf == ILeaf = True
  INode x y z == INode x' y' z' = x == x' && y == y' && z == z'
  _ == _ = False 
------------------------------------------------------------------------------
...
instance Eq a => Eq (List a) where
  Empty == Empty = True
  Empty == _     = False
  LNode x xs == LNode x' xs' = x==x' && xs==xs'
  _ == _ = False
